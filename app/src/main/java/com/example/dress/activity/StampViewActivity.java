...
public class StampViewActivity extends AppCompatActivity {
    ...
    private void initStampList() {
        ...
        private void initStampList_0 () {
            ...
            for (i = 1; i <= 7; i++) {
                int count = 0, sum = 9;
                temp[i] = new Stamp(R.drawable.text_notyet, 1, 0, 1, 10);
                System.out.print(stampList[2][2]);
                System.out.print(stampList[i][3]);
                for (k = 1; k <= 9; k++) {
                    if (stampList[i][k] > 0) {
                        count++;
                        temp[i] = new Stamp(getResource(i, k), i, k, 1,10);
        ...
        private void initStampList_1 ( int group){
            ...
            for (i = 1; i < 10; i++) {
                ...
                if (stampList[group][i] > 0) {
                    temp[i] = new Stamp(getResource(group, i), group, i, stampList[group][i],10);
